...
\begin{itemize}
    ...
    \begin{enumerate}
        %\itemsep-0.3em
      \item Let $w'(u,v)=w(u,v)-\mu(u)-\mu(v)$, where $\mu(v)$ is the cost of the cheapest edge incident to $v$.
      \item Find the minimum weight matching $M$ with $w'$. The answer is $\sum \mu(v) + w'(M)$.
    \end{enumerate}
