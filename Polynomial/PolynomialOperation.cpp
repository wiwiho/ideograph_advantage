...
struct Poly : vector<ll> { // coefficients in [0, P)
  ...
  Poly &shift_inplace(const ll &c) { // SCOPE HASH
    int n = V.n(); // 2 * sz <= MAXN
  ...
  // == PART HASH ==
  vector<ll> _eval(const vector<ll> &x, const vector<Poly> &up) const { // SCOPE HASH
  ...
  static vector<Poly> _tree1(const vector<ll> &x) { // SCOPE HASH
    const int m = (int)x.size();
    vector<Poly> up(m * 2);
  ...
  static Poly Interpolate(const vector<ll> &x, const vector<ll> &y) {	// SCOPE HASH
  ...
  Poly Pow(ll k) const {	// 2*sz<=MAXN // SCOPE HASH
    int nz = 0;
    while (nz < n() && !V[nz]) ++nz;
  ...
  Poly TMul(const Poly &rhs) const { // this[i] - rhs[j] = k; // SCOPE HASH
    return Poly(*this).irev().Mul(rhs).isz(n()).irev();
  }
  Poly FPScomp(Poly g) { // solves V(g(x)) // SCOPE HASH
