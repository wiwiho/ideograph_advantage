...
Pt operator-(Pt a, Pt b){ return {a.X - b.X, a.Y - b.Y}; }
Pt operator*(NumType i, Pt v){ return {i * v.X, i * v.Y}; }
Pt operator/(Pt v, NumType i){ return {v.X / i, v.Y / i}; }
NumType dot(Pt a, Pt b){ return a.X * b.X + a.Y * b.Y; }
NumType cross(Pt a, Pt b){ return a.X * b.Y - a.Y * b.X; }
NumType abs2(Pt v){ return v.X * v.X + v.Y * v.Y; };
int sgn(NumType v){ return v > 0 ? 1 : (v < 0 ? -1 : 0); }
